...
<head>
<link href="css/view_result.css" rel="stylesheet" type="text/css" />
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title>Truth Discovery On Web</title>
</head>
...
<table>
  <tr>
    <td><table width="100%">
        <?php
  $query1 = mysql_query("SELECT rand_id from google_result order by uid DESC limit 1");
  while($qr = mysql_fetch_array($query1))
  {
	  $keywords=$qr['rand_id'];
  }
  ...
  //echo "SELECT * FROM trustworthy_result where uid order by total_score desc and keywords ='$keywords'";
  //exit;
  $query = mysql_query("SELECT * FROM trustworthy_result where rand_id ='$keywords' ORDER BY total_score DESC");
...
/*$page_title_result = mysql_fetch_array(mysql_query("select description as description from trustworthy_result where realurl='".$result["url"]."'"));			
        ...
        <tr>
          <td colspan="2" width="100%"><table border="0" cellpadding="5" cellspacing="0" width="100%">

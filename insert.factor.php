...
$google_table_query = mysql_query("select url from temp_google_result");
//echo mysql_result($google_table_query,1); exit;
$i=0;

	while($google_table_result = mysql_fetch_array($google_table_query))
	{
	/* IIIrd Part = Save all the Factor of Google URL*/
	//$sitehost             = ( $_REQUEST[ 'sendurl' ] ) ? $_REQUEST[ 'sendurl' ] : $_SERVER[ 'HTTP_HOST' ];	
	//$sitehost			  = ($google_table_result['realurl']) ? $google_table_result['realurl']	: $_SERVER[ 'HTTP_HOST' ];
	$url=$google_table_result['url'];
...
if(mysql_affected_rows($chk)!='')
{
	$ret_value =  $i." Records Saved Successfully";
	mysql_query("truncate table temp_google_result") or die(mysql_error());
}
elseif(mysql_affected_rows($chk)=='-1')
{
	
	$ret_value =  $i." Records Save Sucessfully. But, This (".$url.") Not Saved Sucessfully... Please Try Again Later";
}
echo $ret_value;
